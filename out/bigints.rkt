...
(define (add_bigint_positif a b)
  ...
  ((lambda (internal_env) (apply (lambda (retenue chiffres) 
                                        (letrec ([bu (lambda (len) 
                                                       (if (and (> len 0) (eq? (vector-ref chiffres (- len 1)) 0))
                                                       (let ([len (- len 1)])
                                                       (bu len))
                                                       (bigint chiffres len #t)))])
                                        (bu len))) internal_env)) (array_init_withenv len 
  (lambda (i) 
    (lambda (retenue) 
      (let ([tmp retenue])
      ...
      (let ([bs (remainder tmp 10)])
      (list retenue bs)))))))) retenue))))
)
(define (sub_bigint_positif a b)
  ...
  (let ([retenue 0])
  ((lambda (internal_env) (apply (lambda (retenue chiffres) 
                                        (letrec ([br (lambda (len) 
                                                       (if (and (> len 0) (eq? (vector-ref chiffres (- len 1)) 0))
                                                       (let ([len (- len 1)])
                                                       (br len))
                                                       (bigint chiffres len #t)))])
                                        (br len))) internal_env)) (array_init_withenv len 
  (lambda (i) 
    (lambda (retenue) 
      ...
      ((lambda (internal_env) (apply (lambda (retenue tmp) 
                                            (let ([bp tmp])
                                            (list retenue bp))) internal_env)) (if (< tmp 0)
                                                                               (let ([tmp (+ tmp 10)])
                                                                               (let ([retenue (- 1)])
                                                                               (list retenue tmp)))
                                                                               (let ([retenue 0])
