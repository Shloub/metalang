...
(let ([f (quotient (- len 1) 2)])
(letrec ([g (lambda (i) (if (<= i f)
...
(define (print_bigint a)
  (block
    ...
    '())
    (let ([h (- (bigint-bigint_len a) 1)])
    (letrec ([m (lambda (i) (if (<= i h)
                            (block
...
(define (bigint_eq a b)
  ...
  #f
  (let ([o (- (bigint-bigint_len a) 1)])
  (letrec ([p (lambda (i) (if (<= i o)
...
(define (bigint_gt a b)
  ...
  (let ([q (- (bigint-bigint_len a) 1)])
  (letrec ([r (lambda (i) (if (<= i q)
...
(define (add_bigint_positif a b)
  ...
  ((lambda (internal_env) (apply (lambda (retenue chiffres) 
                                        (letrec ([u (lambda (len) (if (and (> len 0) (eq? (vector-ref chiffres (- len 1)) 0))
                                                                  (let ([len (- len 1)])
                                                                  (u len))
                                                                  (bigint chiffres len #t)))])
                                          (u len))) internal_env)) (array_init_withenv len 
  (lambda (i) 
    (lambda (retenue) 
      (let ([tmp retenue])
      ...
      (let ([s (remainder tmp 10)])
      (list retenue s)))))))) retenue))))
)
(define (sub_bigint_positif a b)
  ...
  (let ([retenue 0])
  ((lambda (internal_env) (apply (lambda (retenue chiffres) 
                                        (letrec ([w (lambda (len) (if (and (> len 0) (eq? (vector-ref chiffres (- len 1)) 0))
                                                                  (let ([len (- len 1)])
                                                                  (w len))
                                                                  (bigint chiffres len #t)))])
                                          (w len))) internal_env)) (array_init_withenv len 
  (lambda (i) 
    (lambda (retenue) 
      ...
      ((lambda (internal_env) (apply (lambda (retenue tmp) 
                                            (let ([v tmp])
                                            (list retenue v))) internal_env)) (if (< tmp 0)
                                                                              (let ([tmp (+ tmp 10)])
                                                                              (let ([retenue (- 1)])
                                                                              (list retenue tmp)))
                                                                              (let ([retenue 0])
...
(define (mul_bigint_cp a b)
  ...
  (letrec ([y (lambda (i) (if (<= i x)
                          (let ([retenue 0])
                          (let ([ba (- (bigint-bigint_len b) 1)])
                          (letrec ([bc (lambda (j retenue) (if (<= j ba)
                                                           (block
...
(define (bigint_premiers_chiffres a i)
  ...
  (letrec ([be (lambda (len) (if (and (not (eq? len 0)) (eq? (vector-ref (bigint-bigint_chiffres a) (- len 1)) 0))
                             (let ([len (- len 1)])
                             (be len))
...
(define (log10 a)
  (let ([out0 1])
  (letrec ([bf (lambda (a out0) (if (>= a 10)
                                (let ([a (quotient a 10)])
                                (let ([out0 (+ out0 1)])
                                (bf a out0)))
                                out0))])
...
(define (bigint_of_int i)
  ...
  (let ([t0 (build-vector size (lambda (j) 
                                 0))])
  (letrec ([bg (lambda (k i) (if (<= k (- size 1))
                             (block
...
(define (fact_bigint a)
  ...
  (let ([out0 one])
  (letrec ([bh (lambda (a out0) (if (not (bigint_eq a one))
...
(define (sum_chiffres_bigint a)
  (let ([out0 0])
  (let ([bi (- (bigint-bigint_len a) 1)])
  (letrec ([bj (lambda (i out0) (if (<= i bi)
...
(define (euler48 _)
  ...
  (letrec ([bk (lambda (i sum) (if (<= i 100)
                               ; 1000 normalement 
...
(define (euler25 _)
  ...
  (letrec ([bl (lambda (a b i) (if (< (bigint-bigint_len b) 100)
                               ; 1000 normalement 
...
(define (euler29 _)
  ...
  (let ([n 0])
  (let ([found #t])
  (letrec ([bm (lambda (found n) (if found
                                 (let ([min0 (vector-ref a0_bigint 0)])
                                 (let ([found #f])
                                 (letrec ([bn (lambda (i found min0) (if (<= i maxA)
...
(define main
  (block
    ...
    (letrec ([bp (lambda (i sum) (if (<= i 100)
                                 (block
                                   (mread-blank)
