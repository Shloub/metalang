...
(define (euler29 _)
  ...
  (let ([n 0])
  (let ([found #t])
  (letrec ([bm (lambda (found n) 
                 (if found
                 (let ([min0 (vector-ref a0_bigint 0)])
                 (let ([found #f])
                 (letrec ([bn (lambda (i found min0) 
                                (if (<= i maxA)
