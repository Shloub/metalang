...
let main =
  ...
  (
    ...
    let rec x i =
              ...
              let rec v i_ =
                (if (i_ <= w)
                 ...
                 (
                   ...
                   ) (Array.init_withenv strlen (fun  f p -> Scanf.scanf "%c"
                   (fun  g -> let o = g in
                   ((), o))) ()))
                   )
                 ) in
                (v 0)) (Array.init_withenv len (fun  d m -> Scanf.scanf "%d"
              (fun  e -> (
                           (Scanf.scanf "%[\n \010]" (fun _ -> ()));
                           let l = e in
                           ((), l)
      ...
      (x 0)) (Array.init_withenv len (fun  a k -> Scanf.scanf "%d"
    (fun  b -> (
                 (Scanf.scanf "%[\n \010]" (fun _ -> ()));
                 let h = b in
                 ((), h)
