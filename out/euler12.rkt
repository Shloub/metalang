...
(define (eratostene t0 max0)
  ...
  (letrec ([c (lambda (i n) (if (<= i (- max0 1))
                            ...
                            (let ([j (* i i)])
                            (let ([n (+ n 1)])
                            (letrec ([e (lambda (j) (if (and (< j max0) (> j 0))
                                                    (block
                                                      (vector-set! t0 j 0)
                                                      (let ([j (+ j i)])
                                                      (e j))
...
(define (fillPrimesFactors t0 n primes nprimes)
  (letrec ([f (lambda (i n) (if (<= i (- nprimes 1))
                            ...
                            (letrec ([g (lambda (n) (if (eq? (remainder n d) 0)
                                                    (block
...
(define (find0 ndiv2)
  ...
  (let ([primes (build-vector nprimes (lambda (o) 
                                        0))])
  (let ([l 0])
  (letrec ([h (lambda (k l) (if (<= k (- maximumprimes 1))
                            ...
                            (h (+ k 1) l))
                            (letrec ([p (lambda (n) (if (<= n 10000)
                                                    (let ([primesFactors (build-vector (+ n 2) 
                                                    (lambda (m) 
                                                      0))])
