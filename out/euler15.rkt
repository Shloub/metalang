...
(define main (let ([n 10])
  ...
  (let ([ba 0])
    (let ([bb (- n 1)])
      (letrec ([z (lambda (l n) 
                    (if (<= l bb)
                      (block (vector-set! (vector-ref tab (- n 1)) l 1) (block (vector-set! (vector-ref tab l) (- n 1) 1) (z (+ l 1) n)))
                      (let ([x 2])
                        (let ([y n])
                          (letrec ([s (lambda (o n) 
                                        (if (<= o y)
                                            ...
                                            (let ([v 2])
                                              (let ([w n])
                                                (letrec ([u (lambda (p r n) 
                                                              (if (<= p w)
                                                                (let ([q (- n p)])
                                                                  (block (vector-set! (vector-ref tab r) q (+ (vector-ref (vector-ref tab (+ r 1)) q) (vector-ref (vector-ref tab r) (+ q 1)))) (u (+ p 1) r n)))
                                                                (s (+ o 1) n)))])
                                                (u v r n)))))
                                        (let ([g 0])
                                          (let ([h (- n 1)])
