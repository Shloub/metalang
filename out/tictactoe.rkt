...
(define (print_state g)
  (block
    (display "\n|")
    (letrec ([a (lambda (y) (if (<= y 2)
                            (letrec ([b (lambda (x) (if (<= x 2)
                                                    (block
...
(define (eval0 g)
  (let ([win 0])
  (let ([freecase 0])
  (letrec ([c (lambda (y freecase win) (if (<= y 2)
                                       (let ([col (- 1)])
                                       (let ([lin (- 1)])
                                       (letrec ([e (lambda (x col freecase lin) (if (<= x 2)
                                                                                (let ([freecase 
                                                                                (if (eq? (vector-ref (vector-ref (gamestate-cases g) x) y) 0)
                                                                                (+ freecase 1)
                                                                                freecase)])
...
(define (minmax g)
  (block
    ...
    (let ([maxNote (if (not (gamestate-firstToPlay g))
                   10000
                   maxNote)])
    (letrec ([f (lambda (x maxNote) (if (<= x 2)
                                    (letrec ([h (lambda (y maxNote) (if (<= y 2)
...
(define (play g)
  ...
  (let ([minNote 10000])
  (letrec ([k (lambda (x minNote) (if (<= x 2)
                                  (letrec ([l (lambda (y minNote) (if (<= y 2)
                                                                  (if (can_move_xy x y g)
                                                                  (block
                                                                    (apply_move_xy x y g)
                                                                    (let ([currentNote (minmax g)])
...
(define main
  (letrec ([n (lambda (i) (if (<= i 1)
                          (let ([state (init0 'nil)])
                          (block
                            (apply_move (move 1 1) state)
                            (apply_move (move 0 0) state)
                            (letrec ([o (lambda (_) (if (not (gamestate-ended state))
                                                    (block
                                                      (print_state state)
