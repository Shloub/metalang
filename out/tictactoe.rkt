      ...
      (letrec ([bg (lambda (x freecase win g) 
                     ...
                     (if (and (and (eq? (vector-ref (vector-ref (gamestate-cases g) 0) 0) x) (eq? (vector-ref (vector-ref (gamestate-cases g) 1) 1) x)) (eq? (vector-ref (vector-ref (gamestate-cases g) 2) 2) x))
                       (let ([win x])
                         (bi freecase win g))
                       (bi freecase win g)))
        (block (set-gamestate-ended! g (or (not (eq? win 0)) (eq? freecase 0))) 
        (let ([be (lambda (freecase win g) 
                    '())])
        (if (eq? win 1)
          (block (set-gamestate-note! g 1000) (be freecase win g))
          (let ([bf (lambda (freecase win g) 
                      (be freecase win g))])
          (if (eq? win 2)
            (block (set-gamestate-note! g (- 1000)) (bf freecase win g))
            (block (set-gamestate-note! g 0) (bf freecase win g)))))))))])
(bg bj freecase win g))))))])
(bl bw freecase win g))))))))
(define apply_move_xy (lambda (x y g) 
                        (let ([player 2])
                          (let ([bd (lambda (player x y g) 
                                      (block (vector-set! (vector-ref (gamestate-cases g) x) y player) (block (set-gamestate-firstToPlay! g (not (gamestate-firstToPlay g))) '())))])
...
(define cancel_move_xy (lambda (x y g) 
                         (block (vector-set! (vector-ref (gamestate-cases g) x) y 0) (block (set-gamestate-firstToPlay! g (not (gamestate-firstToPlay g))) (block (set-gamestate-ended! g #f) '())))))
                                                               ...
                                                               (let ([currentNote (minmax g)])
                                                                 (block
                                                                   ...
                                                                   (if (< currentNote minNote)
                                                                    ...
                                                                    (block (set-move-x! minMove x) (block (set-move-y! minMove y) (k currentNote minNote minMove g))))
