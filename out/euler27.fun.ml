...
let main =
  ...
  ((fun  (m, era) -> let result = 0 in
  let max0 = 0 in
  let nprimes = (eratostene era maximumprimes) in
  ((fun  (q, primes) -> let l = 0 in
  ...
  let rec u k l =
    ...
    (u 2 l)) (Array.init_withenv nprimes (fun  o q -> let p = 0 in
  ((), p)) ()))) (Array.init_withenv maximumprimes (fun  j m -> let h = j in
  ((), h)) ()))
